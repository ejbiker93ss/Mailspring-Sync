...
shared_ptr<Message> MailProcessor::insertFallbackToUpdateMessage(IMAPMessage * mMsg, Folder & folder, time_t syncDataTimestamp) {
    // A physical copy has its own UID and folder even when Message-ID, date and
    // recipients match a copy in Sent/Inbox. Prefer that identity on refresh.
    auto physical = store->find<Message>(Query().equal("accountId", folder.accountId())
        .equal("remoteFolderId", folder.id()).equal("remoteUID", (double)mMsg->uid()));
    if (physical) {
        updateMessage(physical.get(), mMsg, folder, syncDataTimestamp);
        return physical;
    }
    try {
        ...
        Query q = Query().equal("accountId", folder.accountId()).equal("remoteFolderId", folder.id())
            .equal("remoteUID", (double)mMsg->uid());
        if (mMsg->gmailMessageID()) q = Query().equal("id", MailUtils::idForMessage(folder.accountId(), folder.path(), mMsg));
...
shared_ptr<Message> MailProcessor::insertMessage(IMAPMessage * mMsg, Folder & folder, time_t syncDataTimestamp) {
    ...
    if (!mMsg->gmailMessageID()) {
        auto existing = store->find<Message>(Query().equal("id", msg->id()));
        if (existing && (existing->remoteFolderId() != folder.id() || existing->remoteUID() != mMsg->uid())) {
            const string base = msg->id();
            msg->_data["physicalBaseId"] = base;
            msg->_data["id"] = base + ":" + folder.id() + ":" + to_string(mMsg->uid());
        }
    }
...
void MailProcessor::repairSmarterMailThread(Message * message, MessageHeader * header, const string & parentMessageId) {
    if (!account->usesSmarterMailAPI() || !header) return;
    // Some API/raw responses HTML-encode IDs. Normalize before matching old
    // cache rows, and never allow a sparse detail response to erase known refs.
    auto normalize = [](String * value) -> string {
        if (!value) return "";
        const string ids = SmarterMailHeaders::ids(value->UTF8Characters());
        return ids.empty() ? "" : ids.substr(1, ids.find('>') - 1);
    };
    const string mid = header->isMessageIDAutoGenerated() ? "" : normalize(header->messageID());
    if (mid.empty() && parentMessageId.empty()) return;
    Array * refs = header->references();
    if (!refs) refs = Array::array();
    Array * normalizedRefs = Array::array();
    for (unsigned int i = 0; i < refs->count(); ++i) {
        const string id = normalize((String *)refs->objectAtIndex(i));
        if (!id.empty()) normalizedRefs->addObject(AS_MCSTR(id));
    }
    refs = normalizedRefs;
    MailStoreTransaction transaction{store, "repairSmarterMailThread"};
    // Refresh in the transaction: the foreground body worker may have healed it.
    auto current = store->find<Message>(Query().equal("id", message->id()));
    if (!current) return;
    set<string> threadIds{current->threadId()};
    if (!parentMessageId.empty()) {
        auto parent = store->find<Message>(Query().equal("accountId", account->id()).equal("id", parentMessageId));
        if (parent && parent->id() != current->id()) {
            threadIds.insert(parent->threadId());
            current->_data["smReplyParentId"] = parent->id();
        }
    }
    // Heal references to older HTML-escaped rows without a mailbox scan.
    vector<string> identities{mid};
    for (unsigned int i = 0; i < refs->count() && i < 50; ++i) identities.push_back(((String *)refs->objectAtIndex(i))->UTF8Characters());
    for (const auto & id : identities) {
        if (id.empty()) continue;
        for (const auto & variant : {id, "<" + id + ">", "&lt;" + id + "&gt;"}) {
            auto matches = store->findAll<Message>(Query().equal("accountId", account->id()).equal("headerMessageId", variant));
            for (const auto & match : matches) threadIds.insert(match->threadId());
        }
    }
    const int count = min(50, (int)refs->count());
    SQLite::Statement matches(store->db(), "SELECT DISTINCT threadId FROM ThreadReference WHERE accountId = ? AND headerMessageId IN (" + MailUtils::qmarks(count + 1) + ")");
    matches.bind(1, account->id());
    matches.bind(2, mid);
    for (int i = 0; i < count; ++i) {
        auto ref = (String *)refs->objectAtIndex(i);
        matches.bind(i + 3, ref ? ref->UTF8Characters() : "");
    }
    while (matches.executeStep()) threadIds.insert(matches.getColumn(0).getString());
    auto target = store->find<Thread>(Query().equal("id", current->threadId()));
    if (!target) return;
    const auto labels = store->allLabelsCache(account->id());
    for (const auto & sourceId : threadIds) {
        if (sourceId == target->id()) continue;
        auto source = store->find<Thread>(Query().equal("id", sourceId).equal("accountId", account->id()));
        if (!source) continue;
        auto messages = store->findAll<Message>(Query().equal("threadId", sourceId).equal("accountId", account->id()));
        for (auto & moved : messages) {
            source->applyMessageAttributeChanges(moved->getSnapshot(), nullptr, labels);
            target->applyMessageAttributeChanges(MessageEmptySnapshot, moved.get(), labels);
            moved->setThreadId(target->id());
            moved->_skipThreadUpdatesAfterSave = true;
            store->save(moved.get());
            appendToThreadSearchContent(target.get(), moved.get(), nullptr);
        }
        SQLite::Statement copyRefs(store->db(), "INSERT OR IGNORE INTO ThreadReference (threadId, accountId, headerMessageId) SELECT ?, accountId, headerMessageId FROM ThreadReference WHERE threadId = ? AND accountId = ?");
        copyRefs.bind(1, target->id()); copyRefs.bind(2, sourceId); copyRefs.bind(3, account->id()); copyRefs.exec();
        SQLite::Statement deleteRefs(store->db(), "DELETE FROM ThreadReference WHERE threadId = ? AND accountId = ?");
        deleteRefs.bind(1, sourceId); deleteRefs.bind(2, account->id()); deleteRefs.exec();
        if (source->searchRowId()) {
            SQLite::Statement searchBody(store->db(), "SELECT body FROM ThreadSearch WHERE rowid = ?");
            searchBody.bind(1, (double)source->searchRowId());
            if (searchBody.executeStep()) {
                const string body = searchBody.getColumn(0).getString();
                appendToThreadSearchContent(target.get(), nullptr, AS_MCSTR(body));
            }
        }
        store->remove(source.get());
    }
    store->save(target.get());
    if (!mid.empty()) {
        upsertThreadReferences(target->id(), account->id(), mid, refs);
        current->_data["hMsgId"] = mid;
    }
    if (refs->count() || !current->_data.count("references")) current->_data["references"] = json::array();
    for (unsigned int i = 0; i < refs->count(); ++i) {
        auto ref = (String *)refs->objectAtIndex(i);
        if (ref && ref->length() > 0) current->_data["references"].push_back(ref->UTF8Characters());
    }
    auto reply = header->inReplyTo();
    if (reply && reply->count()) current->_data["rthMsgId"] = normalize((String *)reply->lastObject());
    store->save(current.get());
    message->_data["hMsgId"] = current->_data["hMsgId"];
    if (current->_data.count("smReplyParentId")) message->_data["smReplyParentId"] = current->_data["smReplyParentId"];
    message->_data["references"] = current->_data["references"];
    message->setThreadId(target->id());
    if (current->_data.count("rthMsgId")) message->_data["rthMsgId"] = current->_data["rthMsgId"];
    transaction.commit();
}
