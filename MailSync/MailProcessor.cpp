...
void MailProcessor::repairSmarterMailThread(Message * message, MessageHeader * header) {
    if (!account->usesSmarterMailAPI() || !header || header->isMessageIDAutoGenerated()) return;
    Array * refs = header->references();
    if (!refs) refs = Array::array();
    const string mid = header->messageID()->UTF8Characters();
    ...
    if (reply && reply->count()) current->_data["rthMsgId"] = ((String *)reply->lastObject())->UTF8Characters();
    store->save(current.get());
    message->_data["hMsgId"] = mid;
