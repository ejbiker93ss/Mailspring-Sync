...
MailModel(MailUtils::idForMessage(folder.accountId(), folder.path(), msg), folder.accountId(), 0)
{
    ...
    _data["headerMessageIdGenerated"] = msg->header()->isMessageIDAutoGenerated();
    ...
    _data["references"] = json::array();
    Array * references = msg->header()->references();
    if (references) {
        for (unsigned int i = 0; i < references->count(); i++) {
            auto reference = (String *)references->objectAtIndex(i);
            if (reference && reference->length() > 0) {
                _data["references"].push_back(reference->UTF8Characters());
            }
        }
    }
