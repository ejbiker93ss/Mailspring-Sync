...
static IMAPMessage * smMessage(const json & remote, const string & folderPath) {
    ...
    const string headers = SmarterMailHeaders::mime(remote) + "\r\n";
    auto parsed = MessageParser::messageParserWithData(Data::dataWithBytes(headers.data(), (unsigned int)headers.size()));
    if (!parsed->header()->isMessageIDAutoGenerated()) header->setMessageID(parsed->header()->messageID());
    header->setReferences(parsed->header()->references());
    header->setInReplyTo(parsed->header()->inReplyTo());
    return message;
}
...
bool SyncWorker::syncSmarterMailMessages()
{
    ...
    for (auto & folder : folders) {
        ...
        do {
            ...
            for (const auto & remote : page.messages) {
                ...
                const string preview = smString(remote, {"preview", "shortPreview", "previewText", "messagePreview"});
                if (!preview.empty()) local->setSnippet(preview.substr(0, 400));
                // The list response is the cheap, server-provided signal that
                // a full RFC822 body will be needed to retain files and inline
                // Content-ID images. Older local bodies were synthesized from
                // text-only detail responses, so mark only attachment-bearing
                // messages for the bounded repair pass below.
                const bool hasAttachments = smBool(remote, {"hasAttachments", "hasAttachment"}) ||
                    (remote.count("attachments") && remote["attachments"].is_array() && !remote["attachments"].empty());
                local->_data["smHasAttachments"] = hasAttachments;
                if (!hasAttachments) local->_data["smAttachmentHydratedV1"] = true;
    ...
    iterationsSinceLaunch += 1;
    // A small, serial background budget fills unopened messages without putting
    // body requests on the interactive worker or fetching an entire mailbox.
    // Attempt timestamps persist across restarts and prevent one bad item from
    // starving older mail. Stop the batch on server/transport errors.
    SQLite::Statement pending(store->db(),
        "SELECT Message.* FROM Message WHERE accountId = ? AND remoteUID > 0 AND remoteUID < 4294967291 "
        "AND (COALESCE(json_extract(data, '$.smThreadHydratedV2'), 0) = 0 "
        "OR (COALESCE(json_extract(data, '$.smHasAttachments'), 0) = 1 "
        "AND COALESCE(json_extract(data, '$.smAttachmentHydratedV1'), 0) = 0)) "
        "AND COALESCE(json_extract(data, '$.smHydrateAttempt'), 0) < ? "
        "ORDER BY date DESC, COALESCE(json_extract(data, '$.smHydrateAttempt'), 0) LIMIT 3");
    pending.bind(1, account->id());
    pending.bind(2, (double)(time(0) - 900));
    vector<shared_ptr<Message>> hydrate;
    while (pending.executeStep()) hydrate.push_back(make_shared<Message>(pending));
    const auto hydrationStart = chrono::steady_clock::now();
    for (auto & message : hydrate) {
        if (chrono::steady_clock::now() - hydrationStart > chrono::seconds(5)) break;
        message->_data["smHydrateAttempt"] = time(0);
        store->save(message.get());
        try {
            syncSmarterMailMessageBody(message.get(), true);
        } catch (const SyncException &) {
            logger->warn("Pausing SmarterMail background hydration after a request failure");
            break;
        }
    }
...
void SyncWorker::syncSmarterMailMessageBody(Message * message, bool background)
{
    ...
    const bool replyWithoutReferences = subject.find("re:") == 0 &&
        (!header->references() || header->references()->count() == 0);
    const bool missingHeaders = header->isMessageIDAutoGenerated() || replyWithoutReferences;
    // Never add compatibility probes to the click-to-body critical path. Older
    // servers omit RFC headers from structured detail; heal those in background.
    if (background && missingHeaders && !body.mime.empty() && !usedRawMime) {
        const string raw = client.rawMessage(path, message->remoteUID());
        if (!raw.empty()) {
            auto rawParser = MessageParser::messageParserWithData(Data::dataWithBytes(raw.data(), (unsigned int)raw.size()));
            if (rawParser && !rawParser->header()->isMessageIDAutoGenerated()) header = rawParser->header();
        }
    }
    string parentId = message->_data.value("smReplyParentId", string());
    string owner = body.replyOwner, expectedOwner = account->emailAddress();
    transform(owner.begin(), owner.end(), owner.begin(), ::tolower);
    transform(expectedOwner.begin(), expectedOwner.end(), expectedOwner.begin(), ::tolower);
    if (body.replyUid && !body.replyFolder.empty() && (owner.empty() || owner == expectedOwner)) {
        auto folder = store->find<Folder>(Query().equal("accountId", account->id()).equal("path", body.replyFolder));
        if (folder) {
            auto parent = store->find<Message>(Query().equal("accountId", account->id())
                .equal("remoteFolderId", folder->id()).equal("remoteUID", (double)body.replyUid));
            if (parent && parent->id() != message->id()) parentId = parent->id();
        }
    }
    processor->repairSmarterMailThread(message, header, parentId);
    message->_data["smHydratedV1"] = background || !missingHeaders;
    message->_data["smAttachmentHydratedV1"] = attachmentMimeLoaded;
    // V1 marked any attempted background recovery successful, even if the raw
    // endpoint returned no references. V2 keeps those rows eligible for bounded
    // retries and upgrades existing caches without rebuilding/downloading all mail.
    message->_data["smThreadHydratedV2"] = !parentId.empty() ||
        (!header->isMessageIDAutoGenerated() && (subject.find("re:") != 0 ||
            (header->references() && header->references()->count() > 0)));
    logger->info("SmarterMail thread recovery UID {}: references={}, parent-pointer={}, complete={}",
        message->remoteUID(), header->references() ? header->references()->count() : 0,
        !parentId.empty(), message->_data["smThreadHydratedV2"].get<bool>());
    store->save(message);
...
void SyncWorker::syncFolderChangesViaCondstore(Folder & folder, IMAPFolderStatus & remoteStatus, bool mustSyncAll)
{
    ...
    for (unsigned int ii = 0; ii < modifiedOrAdded->count(); ii ++) {
        ...
        } else if (msg->gmailMessageID()) {
            string id = MailUtils::idForMessage(folder.accountId(), folder.path(), msg);
            query = Query().equal("id", id);
        } else {
            query = Query().equal("accountId", folder.accountId()).equal("remoteFolderId", folder.id())
                .equal("remoteUID", msg->uid());
        }
